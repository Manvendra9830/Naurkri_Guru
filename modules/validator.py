...
from modules.helpers import normalize_date_posted_value
...
def check_time_hhmm(var: str, var_name: str) -> bool | TypeError | ValueError:
    check_string(var, var_name, min_length=5)
    parts = var.split(":")
    if len(parts) != 2 or not parts[0].isdigit() or not parts[1].isdigit():
        raise ValueError(f'Invalid input for {var_name}. Expected 24-hour time in HH:MM format, example "06:00".')
...
def validate_search() -> None | ValueError | TypeError:
    ...
    normalized_date_posted = normalize_date_posted_value(date_posted, "config/search.py")
    globals()["date_posted"] = normalized_date_posted
    try:
        import config.search as search_config
        search_config.date_posted = normalized_date_posted
    except Exception:
        pass
    check_string(normalized_date_posted, "date_posted", ["", "Any time", "Past month", "Past week", "Past 24 hours"])
...
def validate_secrets() -> None | ValueError | TypeError:
    ...
    # Validate AI provider — supports openai, deepseek, and gemini
...
def validate_settings() -> None | ValueError | TypeError:
    ...
    check_boolean(AUTO_RUN_ENABLED, "AUTO_RUN_ENABLED")
    check_time_hhmm(AUTO_RUN_TIME, "AUTO_RUN_TIME")
    ...
    check_boolean(GMAIL_SYNC_ENABLED, "GMAIL_SYNC_ENABLED")
    check_string(GMAIL_ENV_PATH, "GMAIL_ENV_PATH", min_length=1)
    check_string(GMAIL_IMAP_HOST, "GMAIL_IMAP_HOST", min_length=1)
    check_int(GMAIL_IMAP_PORT, "GMAIL_IMAP_PORT", 1)
    check_int(GMAIL_LOOKBACK_DAYS, "GMAIL_LOOKBACK_DAYS", 1)
    check_int(GMAIL_MAX_EMAILS, "GMAIL_MAX_EMAILS", 1)
    ...
    check_boolean(use_real_chrome_profile, "use_real_chrome_profile")
    check_string(chrome_profile_name, "chrome_profile_name", min_length=1)
    check_boolean(persistent_session, "persistent_session")
